...
class PlayState extends MusicBeatState
{
	...
	public var snowEmitterR:FlxEmitter;
	public var snowEmitterC:FlxEmitter;
	public var snowEmitterL:FlxEmitter;

	public var alphaChange:Bool = false;
	public var holo:FlxSprite;
	public var holoL:FlxSprite;
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			...
			case 'philly': 
					...
					if (FlxG.save.data.newVisuals)
					{
						var trainTrailFX = new FlxTrail(phillyTrain, null, 5, 15, 2, 0.079);
						add(trainTrailFX);
					}
			...
			case 'mall':
			{
					...
					if (FlxG.save.data.newVisuals)
					{
						// defnintiion shit, got all stolen fromm flixel particle demo XD
			...
			default:
			{
					...
					if (FlxG.save.data.newVisuals)
						{
							var holo:FlxSprite = new FlxSprite(350, 10).loadGraphic(Paths.image('holo'));
							holo.setGraphicSize(Std.int(holo.width * 1.2), Std.int(holo.height * 1.54));
							holo.updateHitbox();
							holo.antialiasing = true;
							holo.scrollFactor.set(1, 1);
							holo.offset.set(490.2, 0);
							holo.alpha = 0.6;
							add(holo);

							var holoL:FlxSprite = new FlxSprite(1250, 380).loadGraphic(Paths.image('holo'));
							holoL.setGraphicSize(Std.int(holo.width * 1.2), Std.int(holo.height * 1.54));
							holoL.updateHitbox();
							holoL.antialiasing = true;
							holoL.scrollFactor.set(1.1, 1.1);
							holoL.offset.set(490.2, 0);
							holoL.alpha = 0.6;
							add(holoL);

							FlxTween.angle(holo, -16, 20, 1.5, {ease : FlxEase.cubeInOut, type : PINGPONG});
							FlxTween.color(holo, 3, FlxColor.fromRGB(255, 255, 255, 200), FlxColor.fromRGB(255, 9, 176, 87), {ease : FlxEase.quadInOut, type : PINGPONG});
							//FlxTween.tween(holo, {alpha : 0.1}, 0.5,{ease:FlxEase.cubeInOut, type: PINGPONG});

							//FlxTween.tween(holo, {alpha:0})

							FlxTween.angle(holoL, 23, -20, 1.5, {ease : FlxEase.cubeInOut, type : PINGPONG});
							FlxTween.color(holoL, 3, FlxColor.fromRGB(255, 255, 255, 200), FlxColor.fromRGB(176, 9, 255, 87), {ease : FlxEase.quadInOut, type : PINGPONG});
							//FlxTween.tween(holoL, {alpha : 0.1}, 0.5,{ease:FlxEase.cubeInOut, type: PINGPONG}); 
						}
		...
		scoreTxt.setFormat(Paths.font("phantom.otf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		replayTxt.setFormat(Paths.font("phantom.otf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		if (!FlxG.save.data.newVisuals)
		{
			snowEmitterR.x = camPos.x;
			snowEmitterC.x = camPos.x;
			snowEmitterL.x = camPos.x;	
		}
		...
		if(FlxG.save.data.distractions){
			if (trainSound.time >= 4700)
				...
				if (startedMoving)
				{
					...
					if (FlxG.save.data.newVisuals)
						camGame.shake(0.00425, 0.0925, null, true);
                    	//camHUD.shake(0.08525, 0.0925, null, true);
						//appShakeShit(0.00425, 0.0925);
